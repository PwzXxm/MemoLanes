...
use crate::achievement::region_read;
pub use crate::achievement::region_read::{
    RegionDetail, RegionEntry, RegionKind, RegionLayerStat, RegionLevel, RegionLevelView,
    RegionNode,
};
...
// Regions (layered): FRB entry points over `achievement::region_read`.

pub fn region_levels() -> Result<Vec<RegionLevel>> {
    crate::api::api::get()
        .storage
        .with_achievement_read(|store| {
            Ok(store
                .geo()
                .map_or_else(Vec::new, region_read::region_levels))
        })
}
...
pub fn region_level_view(
    ...
    crate::api::api::get()
        ...
        .with_achievement_read(|store| {
            ...
            Ok(region_read::region_level_view(
                &store.region_states()?,
                geo,
                layer,
                level,
...
pub fn region_detail(entity_id: u32) -> Result<Option<RegionDetail>> {
    crate::api::api::get()
        .storage
        .with_achievement_read(|store| {
            let Some(geo) = store.geo() else {
