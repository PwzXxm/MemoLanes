...
pub trait GeoLookup {
    ...
    /// Worldviews embedded in this asset (the full shipped POV list). Runtime
    /// reads this, never `Pov::shipped_worldviews()` — under geo-C the embedded
...
impl GeoLookup for GeoIndex {
    fn entity_of_block(&self, tile: TileKey, block: BlockKey) -> Option<GeoEntityId> {
        match self.tile_entry(tile) {
            ...
            // Decode-on-demand; an LRU over decoded tiles is a deferred optimization.
            TileEntry::Border(i) => {
                let packed =
