//! End-to-end over the FRB-facing achievement API (`src/api/achievement.rs`),
...
fn api_achievement_explored_area_and_region_contract() {
    ...
    assert_eq!((view.visited, view.total), (0, 0));
    assert!(view.entries.is_empty());
    assert_eq!(view.level, RegionKind::Country);
    assert!(region_detail(1).unwrap().is_none(), "no geo → no detail");
