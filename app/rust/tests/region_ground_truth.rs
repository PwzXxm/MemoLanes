...
use memolanes_core::{
    achievement::region_index::RegionStateMap,
...
fn region_api_reports_correct_countries_and_areas() {
    ...
    let states = storage
        .with_achievement_read(|s| s.region_states())
        .unwrap();
    let states = &states;
    ...
    storage
        .with_achievement_read(|store| {
            ...
            let country_total = levels
                .iter()
                .find(|l| l.kind == RegionKind::Country)
                .unwrap()
                .total;
            assert!(
                country_total > 100,
            ...
            assert!(levels.iter().any(|l| l.kind == RegionKind::Continent));
            ...
            let fr_entry = view
                .entries
                .iter()
                .find(|e| e.entity_id == fr.0)
                .expect("FR listed");
            assert!(fr_entry.first_visit_date.is_some(), "FR should be visited");
            assert!(view.visited >= 1 && view.visited <= view.total);

            // Detail of France: area matches the states it was built from.
            let detail = region_detail(&states, geo, fr.0).unwrap();
            assert_eq!(detail.node.entity_id, fr.0);
            assert_eq!(
                detail.node.by_layer[&Default].visited_area_m2,
