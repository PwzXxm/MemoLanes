...
use memolanes_core::{
    achievement::region_read::{region_detail, region_level_view, region_levels, RegionKind},
...
fn region_read_api_lists_progress_and_completion() {
    ...
    storage
        .with_achievement_read(|store| {
            ...
            let states = store.region_states()?;

            // Levels: 1 continent, 2 countries.
            let levels = region_levels(geo);
            assert!(levels
                .iter()
                .any(|l| l.kind == RegionKind::Continent && l.total == 1));
            assert!(levels
                .iter()
                .any(|l| l.kind == RegionKind::Country && l.total == 2));

            let eu = Some(1u32);
            // Entries always list every country (FR, DE); only visited ones carry
            // a first-visit date. Default sees only FR visited; All sees both.
            let def = region_level_view(&states, geo, Default, RegionKind::Country, eu);
            let mut def_ids: Vec<_> = def.entries.iter().map(|e| e.entity_id).collect();
            def_ids.sort();
            assert_eq!(def_ids, vec![2, 3]);
            let mut def_visited: Vec<_> = def
                .entries
                .iter()
                .filter(|e| e.first_visit_date.is_some())
                .map(|e| e.entity_id)
                .collect();
            def_visited.sort();
            assert_eq!(def_visited, vec![2]);

            // Counts + completion badge.
            assert_eq!((def.visited, def.total), (1, 2));
            assert!(def.completed_at.is_none());
            let all = region_level_view(&states, geo, All, RegionKind::Country, eu);
            assert_eq!((all.visited, all.total), (2, 2));
            assert!(all.completed_at.is_some());

            // Detail of EU: per-layer node + FR/DE children.
            let detail = region_detail(&states, geo, 1).unwrap();
            assert_eq!(detail.node.entity_id, 1);
            assert!(detail.node.by_layer[&All].visited_area_m2 > 0);
            let mut kids: Vec<_> = detail.children.iter().map(|c| c.entity_id).collect();
            kids.sort();
